...
function App() {
  ...
  // DB products fetched from the API — merged on top of the static demo catalogue
  const [apiProducts, setApiProducts] = useState([])
  const restoringHistory = useRef(false)
  const activeSlide = slides[slide]

  // Merge static demo products with any DB products that don't share a name with a static entry.
  // This way the local-asset visuals are preserved for the demo, and newly listed items appear too.
  const allProducts = [
    ...products,
    ...apiProducts.filter((dbProduct) => !products.some((staticProduct) => staticProduct.name === dbProduct.name)),
  ]
  ...
  return <>
    ...
    <main>
      ...
      {page === 'Home' && <section className="sell-section"><div><p className="eyebrow">YOUR CLOSET HAS VALUE</p><h2>Pass it on. Get paid.</h2></div><button className="light-button" type="button" onClick={() => { setSellFormSubmitted(false); setSellFormError(''); setSellFormOpen(true) }}>Start selling <span>→</span></button></section>}
    ...
    {sellFormOpen && <div className="sell-form-overlay" role="presentation" onClick={() => setSellFormOpen(false)}><section className="sell-form-dialog" role="dialog" aria-modal="true" aria-labelledby="sell-form-title" onClick={(event) => event.stopPropagation()}><button className="close-sell-form" type="button" onClick={() => setSellFormOpen(false)} aria-label="Close sell form">×</button>{sellFormSubmitted ? <div className="sell-form-success" role="status"><span>✓</span><h2 id="sell-form-title">Request submitted!</h2><p>Thanks for sharing your item. We’ll contact you by email or phone after reviewing the details.</p><button className="primary-button" type="button" onClick={() => setSellFormOpen(false)}>Done <span>→</span></button></div> : <><p className="eyebrow">SELL WITH RETROFIT</p><h2 id="sell-form-title">Tell us about your item</h2><p className="sell-form-note">Complete these details and we’ll help your item find its next home.</p>{!user && <p className="sell-form-note" style={{color:'var(--accent,#c8a96e)',marginTop:0}}>You need to sign in before listing an item.</p>}<form className="sell-request-form" onSubmit={async (event) => { event.preventDefault(); if (!user) { setSellFormOpen(false); setSignInOpen(true); return } const form = event.target; setSellFormLoading(true); setSellFormError(''); try { const token = localStorage.getItem('token'); await createProduct({ name: form.productName.value, image: form.imageUrl.value || 'https://images.unsplash.com/photo-1523381210434-271e8be1f52b?auto=format&fit=crop&w=800&q=80', group: form.group.value, size: form.size.value, usedFor: form.usedFor.value, price: Number(form.price.value), condition: form.condition.value, style: 'Western', }, token); setSellFormSubmitted(true); fetchApiProducts() } catch (error) { setSellFormError(error.message || 'Something went wrong. Please try again.') } finally { setSellFormLoading(false) } }}><label><span>1. What would you like to sell?</span><input name="productName" type="text" placeholder="Write the product name" required /></label><label><span>2. Category</span><select name="group" required defaultValue=""><option value="" disabled>Select category</option><option value="Women">Women</option><option value="Men">Men</option><option value="Kids">Kids</option></select></label><label><span>3. Size</span><input name="size" type="text" placeholder="e.g. M, L, Free size, 32" required /></label><label><span>4. Image URL <small>(optional public photo link)</small></span><input name="imageUrl" type="url" placeholder="https://..." /></label><label><span>5. How long have you used it?</span><select name="usedFor" required defaultValue=""><option value="" disabled>Select duration</option><option value="Used for 15 days">Less than 1 month</option><option value="Used for 1 month">1 month</option><option value="Used for 2 months">2–3 months</option><option value="Used for 4 months">4–6 months</option><option value="Used for 6 months">More than 6 months</option></select></label><label><span>6. Condition</span><select name="condition" required defaultValue=""><option value="" disabled>Select condition</option><option value="Like New">Like New</option><option value="Good">Good</option><option value="Fair">Fair</option></select></label><label><span>7. What price would you like? (৳)</span><input name="price" type="number" min="1" inputMode="numeric" placeholder="Enter amount in ৳" required /></label>{sellFormError && <p className="auth-error">{sellFormError}</p>}<button className="primary-button" type="submit" disabled={sellFormLoading}>{sellFormLoading ? 'Submitting…' : user ? 'List item' : 'Sign in to list'} <span>→</span></button></form></>}</section></div>}
